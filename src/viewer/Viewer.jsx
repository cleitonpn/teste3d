...
function OrbitRig({ bounds }) {
  ...
  return (
    <OrbitControls
      ref={ctrl} makeDefault enableDamping dampingFactor={0.08}
  ...
  return (
    <div style={{ position: 'fixed', inset: 0 }}>
      ...
      >
        ...
        <directionalLight
          position={[7, 15, 7]} intensity={2.4} castShadow
          ...
          <>
            <mesh rotation={[-Math.PI / 2, 0, 0]} position={[bounds.center.x, bounds.min.y - 0.015, bounds.center.z]} receiveShadow>
              ...
              <MeshReflectorMaterial
                resolution={quality === 'high' ? 1024 : 512} mixBlur={6}
                mixStrength={quality === 'high' ? 1.4 : 0.8} blur={[400, 200]}
                roughness={0.85} depthScale={0.8} minDepthThreshold={0.4} maxDepthThreshold={1.2}
                color="#e0dcd3" metalness={0.15}
              />
            </mesh>
      ...
      {mode === 'walk' && (
        <div style={{ position: 'fixed', left: '50%', top: '50%', width: 8, height: 8, marginLeft: -4, marginTop: -4,
          borderRadius: '50%', background: 'rgba(255,255,255,0.7)', boxShadow: '0 0 0 2px rgba(0,0,0,0.35)',
          pointerEvents: 'none', zIndex: 10 }} />
      )}

      <div style={{ position: 'fixed', right: 16, top: 16, zIndex: 30, display: 'flex', gap: 8 }}>
        <button onClick={() => setQuality((q) => (q === 'high' ? 'balanced' : 'high'))} style={btnStyle}>
          {quality === 'high' ? '✨ Alta' : '⚡ Equilibrada'}
        </button>
        <button onClick={() => setMode((m) => (m === 'orbit' ? 'walk' : 'orbit'))} style={btnStyle}>
          {mode === 'orbit' ? '🔄 Órbita' : '🚶 Pessoa'}
        </button>
      </div>

      {showHint && (
        <div style={{ position: 'fixed', left: '50%', top: 22, transform: 'translateX(-50%)',
          background: 'rgba(15,20,26,0.82)', color: '#fff', padding: '10px 16px', borderRadius: 999,
          fontSize: 14, zIndex: 15, textAlign: 'center', maxWidth: '92vw', border: '1px solid rgba(255,255,255,0.12)' }}>
          {isTouch
            ? (mode === 'orbit' ? '👆 1 dedo gira  •  ✌️ 2 dedos zoom / mover' : '👆 Arraste para olhar  •  🕹️ joystick para andar')
            : (mode === 'orbit' ? '🖱️ Arraste = girar  •  Scroll = zoom  •  Botão direito = mover  •  V = pessoa' : '🖱️ Clique para olhar  •  WASD para andar  •  V = órbita')}
        </div>
      )}
